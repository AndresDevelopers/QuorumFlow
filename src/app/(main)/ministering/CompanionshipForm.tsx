...
import { addDoc, updateDoc, doc, getDocs, query, orderBy } from 'firebase/firestore';
...
export function CompanionshipForm({ companionship, onCancel }: CompanionshipFormProps) {
  ...
  const [companionEntryMode, setCompanionEntryMode] = useState<'manual' | 'automatic'>('manual');
  const [familyEntryMode, setFamilyEntryMode] = useState<'manual' | 'automatic'>('manual');
      ...
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <Card>
          ...
          <CardContent className="space-y-6">
            {/* Companion Entry Mode Selection - Only show in add mode */}
            {!isEditMode && (
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <div className="flex items-center gap-2">
                        {companionEntryMode === 'automatic' && !isEditMode ? (
            ...
            {/* Family Entry Mode Selection - Only show in add mode */}
            {!isEditMode && (
              <div className="space-y-4">
                <div>
              ...
              {familyFields.map((field, index) => (
                 <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <div className="flex items-center gap-2">
                        {familyEntryMode === 'automatic' && !isEditMode ? (
                        ...
                        <Button type="button" variant="outline" size="icon" onClick={() => removeFamily(index)} disabled={familyFields.length <= 1}>
                            <Trash2 className="h-4 w-4" />
                        </Button>
                      </div>
                      <FormMessage />
