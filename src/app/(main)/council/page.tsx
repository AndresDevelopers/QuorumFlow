...
import { convertsCollection, futureMembersCollection, ministeringCollection, annotationsCollection, servicesCollection, activitiesCollection } from '@/lib/collections';
import type { Convert, FutureMember, Companionship, Family, Annotation, Service, Member, Activity } from '@/lib/types';
...
import { format, subMonths, addDays, subHours, isAfter, isBefore } from 'date-fns';
...
async function getCouncilConverts(): Promise<Convert[]> {
  const eighteenMonthsAgo = subMonths(new Date(), 18);
  const twentyFourHoursAgo = subHours(new Date(), 24);

  const snapshot = await getDocs(query(convertsCollection, orderBy('baptismDate', 'desc')));
  
  const councilList = snapshot.docs
    .map(doc => ({ id: doc.id, ...doc.data() } as Convert))
    .filter(convert => {
        const baptismDate = convert.baptismDate.toDate();
        if (baptismDate < eighteenMonthsAgo) return false;
