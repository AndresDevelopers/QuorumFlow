...
import { createMember, updateMember, uploadMemberPhoto, uploadBaptismPhotos, getMembersForSelector } from '@/lib/members-data';
...
export function MemberForm({ member, onClose }: MemberFormProps) {
  ...
  const onSubmit = async (values: MemberFormValues) => {
    ...
    try {
      ...
      // Verificar si es un bautismo del año actual
      const currentYear = new Date().getFullYear();
      const isRecentBaptism = values.baptismDate && 
        values.baptismDate.getFullYear() === currentYear;
      ...
      const memberData: Partial<Omit<Member, 'id' | 'createdAt' | 'createdBy'>> = {
        ...
        // Conservar datos existentes o usar nuevos valores
        phoneNumber: values.phoneNumber?.trim() || member?.phoneNumber || undefined,
        birthDate: values.birthDate ? Timestamp.fromDate(values.birthDate) : member?.birthDate,
        baptismDate: values.baptismDate ? Timestamp.fromDate(values.baptismDate) : member?.baptismDate,
        photoURL: photoURL || member?.photoURL || undefined,
      ...
      if (isRecentBaptism && values.baptismDate) {
        ...
        const { addDoc, collection, query, where, getDocs, deleteDoc, doc } = await import('firebase/firestore');
        ...
        // Crear nuevo registro si hay fecha de bautismo en el año actual
        if (isRecentBaptism && values.baptismDate) {
          await addDoc(collection(firestore, 'converts'), convertData);
          await addDoc(collection(firestore, 'c_bautismos'), baptismData);
        }
      }

      if (member) {
        // Obtener ministrantes previos para sincronización
        let previousTeachers: string[] = [];
        try {
          previousTeachers = await getPreviousMinisteringTeachers(member.id);
        } catch (error) {
          console.warn('Warning: Could not get previous ministering teachers:', error);
          previousTeachers = member.ministeringTeachers || [];
        }
        
        // Update existing member
        await updateMember(member.id, memberData);
        
        // Sincronizar asignaciones de ministración si hay cambios en los ministrantes
        try {
          const currentTeachers = values.ministeringTeachers || [];
          const hasTeacherChanges = 
            JSON.stringify(previousTeachers.sort()) !== JSON.stringify(currentTeachers.sort());
            
          if (hasTeacherChanges) {
            const updatedMember = { ...member, ...memberData } as Member;
            await syncMinisteringAssignments(updatedMember, previousTeachers);
            console.log('🔄 Ministering assignments synced for updated member');
          }
        } catch (syncError) {
          console.warn('⚠️ Warning: Could not sync ministering assignments:', syncError);
          // No lanzar error para no interrumpir el guardado del miembro
        }
        ...
        // Create new member - ensure all required fields are provided
        const newMember: Omit<Member, 'id'> = {
          ...
          ordinances: values.ordinances || [],
          ministeringTeachers: values.ministeringTeachers || [],
          lastActiveDate: values.status === 'active' ? Timestamp.now() : undefined,
          inactiveSince: values.status !== 'active' ? Timestamp.now() : undefined,
        };
        const newMemberId = await createMember(newMember);
        
        // Sincronizar asignaciones de ministración para nuevo miembro si tiene ministrantes
        try {
          const currentTeachers = values.ministeringTeachers || [];
          if (currentTeachers.length > 0) {
            const memberWithId = { ...newMember, id: newMemberId } as Member;
            await syncMinisteringAssignments(memberWithId, []);
            console.log('🔄 Ministering assignments synced for new member');
          }
        } catch (syncError) {
          console.warn('⚠️ Warning: Could not sync ministering assignments for new member:', syncError);
          // No lanzar error para no interrumpir el guardado del miembro
        }
