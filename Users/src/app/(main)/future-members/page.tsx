...
export default function FutureMembersPage() {
  ...
  const handleDelete = async (member: FutureMember) => {
    try {
      if (member.photoURL) {
        try {
            ...
            if (storageError.code !== 'storage/object-not-found') {
                logger.warn({ error: storageError, message: 'Could not delete photo from storage.'});
            }
        }
      }
      await deleteDoc(doc(futureMembersCollection, member.id));
      toast({
      ...
      logger.error({ error, message: 'Error deleting future member', futureMemberId: member.id });
      toast({
        title: 'Error',
