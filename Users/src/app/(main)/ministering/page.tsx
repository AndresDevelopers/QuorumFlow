    ...
    try {
        ...
        if (!isSameMonth(now, lastReset)) {
            startResetting(async () => {
                ...
                const firestore = doc(ministeringCollection).firestore;
      ...
      logger.error({error: error instanceof Error ? error.message : String(error)}, "Failed to fetch ministering data");
            ...
            <div className="hidden md:block">
                ...
                <TableBody>
                    ...
                    <TableRow key={item.id} className={item.families.some(f => f.isUrgent) ? 'bg-destructive/10' : ''}>
                        <TableCell className="font-medium">
                          <div className="flex flex-wrap items-center gap-x-2">
                            {item.companions.map((companion, index) => (
                                <Fragment key={index}>
                                    <span>{companion}</span>
                                    {index < item.companions.length - 1 && <span className="text-muted-foreground">|</span>}
                                </Fragment>
                            ))}
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex flex-wrap items-center gap-x-2">
                            {item.families.map((family, index) => (
                                <Fragment key={index}>
                                    <span>{family.name}</span>
                                    {index < item.families.length - 1 && <span className="text-muted-foreground">|</span>}
                                </Fragment>
                            ))}
                          </div>
                        </TableCell>
